...
window.addEventListener('load', () => {
    setTimeout(() => {
        showToast("Permission is granted!", "Welcome to KYAMC Cultural Week 2026 Portal. Curiosity → Questions → Learning → Understanding → Growth ");
...
/* --- UPDATED IDENTITY MATRIX LOGIC --- */

function updateCard() {
    // 1. Get Elements
    const nameInput = document.getElementById('idName').value;
    const batchInput = document.getElementById('idBatch').value;
    const rollInput = document.getElementById('idRoll');
    const houseInput = document.getElementById('idHouse').value;
    
    // Card Elements
    const cardName = document.getElementById('cardName');
    const cardBatch = document.getElementById('cardBatch');
    const cardRoll = document.getElementById('cardRoll');
    const cardHouse = document.getElementById('cardHouse');
    
    // 2. Update Name
    cardName.innerText = nameInput ? nameInput : "NAME";
    
    // 3. Logic: Student vs Faculty
    if (batchInput === 'Faculty') {
        // HIDE Roll Number features
        rollInput.style.display = 'none'; // Hide Input
        cardRoll.style.display = 'none'; // Hide Text on Card
        
        cardBatch.innerText = "DESIGNATION: FACULTY";
        cardBatch.style.color = "#f59e0b"; // Gold color for teachers
    } else {
        // SHOW Roll Number features
        rollInput.style.display = 'block'; // Show Input
        cardRoll.style.display = 'block'; // Show Text on Card
        
        cardBatch.innerText = "BATCH: " + batchInput;
        cardBatch.style.color = "#ccc";
        
        // Update Roll Text
        const rollVal = rollInput.value;
        cardRoll.innerText = rollVal ? "ROLL: " + rollVal : "ROLL: 00";
    }
    
    // 4. Update House Badge Color/Text
    cardHouse.innerText = houseInput.toUpperCase();
    
    // Remove old classes
    cardHouse.classList.remove('balaka-bg', 'progoti-bg', 'atlas-bg', 'oikantik-bg');
    
    // Add new class
    cardHouse.classList.add(houseInput + '-bg');
}
...
function downloadIdCard() {
    ...
    html2canvas(cardElement, {
        ...
        link.download = 'KYAMC_CW26_ID_Card.png';
        ...
        if (typeof showToast === "function") {
            setTimeout(() => {
                showToast("Download Completed", "Holographic  CW'26 I'd card saved to your gallery.");
            }, 1000);
        }
    });
}
